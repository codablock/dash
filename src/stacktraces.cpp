...
static __attribute__((noinline)) std::vector<uintptr_t> GetStackFrames(size_t skip, size_t max_frames)
{
    ...
    backtrace_simple(GetLibBacktraceState(), (int)skip, my_backtrace_simple_callback, my_backtrace_error_callback, &v);
    return v;*/
}
#endif
...
static __attribute__((noinline)) int GetCrashType()
{
    // exclude crash type 5 (stack overflow) as we can't handle it propery in linux (including Windows on Wine)
    // If you want to test it, just change this line to return the fixed number 5
    return GetRandInt(5);
}

static __attribute__((noinline)) int DoCrashRecursiveLoop()
{
    std::string a("1"), b("2"), c("3"), d, e, f;
    a += b + c + d + e + f;
    if (a.empty()) {
        // will never happen
        LogPrintf( "%s", a);
...
static __attribute__((noinline)) void DoCrash2()
{
    static int zero = 0;
    static int test = 1;

    int rnd = GetCrashType();
    switch (rnd) {
        case 0:
        ...
        case 5: {
            DoCrashRecursiveLoop();
            break;
        }
    }
};

static __attribute__((noinline)) void DoCrash()
{
    DoCrash2();
}

void __attribute__((noinline)) TestCrash(bool allowOtherThread)
{
    bool otherThread = allowOtherThread && (GetRandInt(2)) == 0;

    if (otherThread) {
        std::thread t(DoCrash);
        if (t.joinable()) {
            t.join();
        }
    } else {
